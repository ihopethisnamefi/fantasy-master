...
def getSmallestMarginVictory():
    "Return Smallest Margin of Victory in a Game between both Managers"
    c.execute('SELECT manager, matchupTotal, score, year, week, vs, winLoss, MIN(ABS(pointDiff)) FROM matchups WHERE winLoss = "win"'.\
            format(tn=matchupData))
    top_game = c.fetchone()
    return (str(top_game[0]), top_game[1], top_game[2], top_game[3], str(top_game[4]), str(top_game[5]), str(top_game[6]), top_game[7])

def getBiggestMarginVictory():
    "Return Biggest Margin of Victory in a Game between both Managers"
    c.execute('SELECT manager, matchupTotal, score, year, week, vs, winLoss, MAX(ABS(pointDiff)) FROM matchups WHERE winLoss = "win"'.\
            format(tn=matchupData))
    top_game = c.fetchone()
    return (str(top_game[0]), top_game[1], top_game[2], top_game[3], str(top_game[4]), str(top_game[5]), str(top_game[6]), top_game[7])
...
def getMostWinsSeason():
    "Return Most Wins in a Season"
    c.execute('SELECT manager, year, count(winLoss) FROM {tn} where winLoss = "win" and week < 14 group by manager, year order by count(winLoss) DESC LIMIT 1'.\
            format(tn=matchupData))
    top_game = c.fetchone()
    return (str(top_game[0]), str(top_game[1]), top_game[2])

def getLeastWinsSeason():
    "Return Least Wins in a Season"
    c.execute('SELECT manager, year, count(winLoss) FROM {tn} where winLoss = "win" and week < 14 group by manager, year order by count(winLoss) ASC LIMIT 2'.\
            format(tn=matchupData))
    top_game = c.fetchall()
    return (top_game)
...
sMV = getSmallestMarginVictory()
smvString = ""
smvOppScore = sMV[1] - sMV[2];
smvString = str(sMV[0]) + " (" + str(sMV[2]) + ") vs. " + str(sMV[5]) + " (" + str(smvOppScore) + ")"

bMV = getBiggestMarginVictory()
bmvString = ""
bmvOppScore = bMV[1] - bMV[2];
bmvString = str(bMV[0]) + " (" + str(bMV[2]) + ") vs. " + str(bMV[5]) + " (" + str(bmvOppScore) + ")"

mWS = getMostWinsSeason()
mwsString = ""

lWS = getLeastWinsSeason()
lwsString = ""
...
resultString = resultString + "<tr> <td class='recordType even'>Fewest Points (Game) </td><td class='center even'>" + str(lPG[1]) + "</td><td class='center even' title='" + str(lPG[2]) + " - Week " + str(lPG[3]) + "'>" + str(lPG[0]) + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType odd'>Fewest Points (Season) </td><td class='center odd'>" + str(lPS[2]) + "</td><td class='center odd' >" + str(lPS[0]) + " - " + str(lPS[1]) + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType even'>Fewest Points (Match) </td><td class='center even'>" + str(lPM[1]) + "</td><td class='center even' title='" + str(lPM[3]) + " - Week " + str(lPM[4]) + "'>" + leastMatchupString + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType odd'>Most Points Allowed (Season) </td><td class='center even'>" + str(mPAS[2]) + "</td><td class='center even' >" + str(mPAS[0]) + " - " + str(mPAS[1]) + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType even'>Fewest Points Allowed (Season) </td><td class='center even'>" + str(fPAS[2]) + "</td><td class='center even' >" + str(fPAS[0]) + " - " + str(fPAS[1]) + "</td></tr>"
...
resultString = resultString + "<tr> <td class='recordType odd'>Most Points-Player (Game) </td><td class='center odd'>" + str(mPPlG[2]) + "</td><td class='center odd' title='" + str(mPPlG[3]) + " - Week " + str(mPPlG[4]) + "; Owner: " + str(mPPlG[5]) + "'>" + str(mPPlG[0]) + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType even'>Most Points-Player (Season) </td><td class='center even'>" + str(mPPlS[2]) + "</td><td class='center even'>" + str(mPPlS[0]) + " - " + str(mPPlS[1]) + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType odd'>Smallest Margin Of Victory </td><td class='center odd'>" + str(sMV[7]) + "</td><td class='center odd' title='" + str(sMV[3]) + " - Week " + str(sMV[4]) + " AND 2016 - Week 9'>" + smvString + "<br>Jon Thibeault (101.1) vs. Jeremy Hsu (100.9)</td></tr>"
resultString = resultString + "<tr> <td class='recordType even'>Biggest Blowout </td><td class='center even'>" + str(bMV[7]) + "</td><td class='center even' title='" + str(bMV[3]) + " - Week " + str(bMV[4]) + "'>" + bmvString + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType odd'>Most Wins (Season) </td><td class='center odd'>" + str(mWS[2]) + "</td><td class='center odd' >" + str(mWS[0]) + " - " + str(mWS[1]) + "</td></tr>"
resultString = resultString + "<tr> <td class='recordType even'>Least Wins (Season) </td><td class='center even'>" + str(lWS[0][2]) + "</td><td class='center even' >" + str(lWS[0][0]) + " - " + str(lWS[0][1]) + "<br>" + str(lWS[1][0]) + " - " + str(lWS[1][1]) + "</td></tr>"
