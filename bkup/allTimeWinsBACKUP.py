...
def getWorkSacko(year):
    "Return League Sacko for Given Year"
    c.execute('SELECT manager, year, week, winLoss, score FROM {tn} WHERE year = {yr} AND week = 17 AND winLoss = "loss" ORDER BY manager ASC, year ASC, week ASC'.\
            format(tn=matchupData, yr=year))
    sacko = c.fetchone()
    if sacko is None:
        return "N/A"
...
def getNumPlayoffAppearences(manager):
    ...
    c.execute('SELECT manager, year, week FROM {tn} WHERE manager = "{mn}" AND week > 13 AND week < 17 ORDER BY year ASC, week ASC'.\
